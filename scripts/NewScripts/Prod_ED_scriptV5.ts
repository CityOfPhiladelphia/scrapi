...
const api_config = {
    summary_URL: "https://xpyab0tpx5.execute-api.us-east-1.amazonaws.com/prod/usjs/v1/summary",
    docket_URL: "https://xpyab0tpx5.execute-api.us-east-1.amazonaws.com/prod/usjs/v1/docket"
} as const;
...
function getCaseStatus(cases: Case[] | undefined, docketNum: string, financialCaseStatus?: string): string {
    const docketCase = getCasesForDocket(cases, docketNum)[0];
    const procStatus = (docketCase?.procStatus || "").trim();

    if (/^archived$/i.test(procStatus)) {
        return "Archived";
    }

    return financialCaseStatus || "";
}
...
class SheetPopulator {
    ...
    populateCaseData(docketNum: string, data: ProcessedData): void {
        ...
        // Case status
        const caseStatus = getCaseStatus(summary?.cases, docketNum, financial?.casestatus);
        this.setCell(columns.case_status, caseStatus);
